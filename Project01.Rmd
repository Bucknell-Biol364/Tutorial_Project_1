...
#####Target audiance is any ecology/biology student who has taken field data, the goal is to clean up field data so it is in a useable form for analysis.


```{r}
summer21_data <- read.csv(file = 'summer21UNclean.csv')
print(summer21_data)
...
####upon looking at the data, it is clear there are unnecessary spaces between datasets, column headers, and the data is wide instead of long. We will first start by removing the spaces between datasets and column headers. gsub is a great way to remove unwanted spaces in strings of data. 
...
#### this worked to remove the extra spacing between the data sets. Now we will focus on changing our data from wide data to long data.

```{r}
gather(summer21cleaned)
```

###The code below is used to explaim how to create a graph with two variables. Here we are using ggplot to show how Absorbance and Phosphorus levels interact.
```{r}
 ggplot(summer21clean, aes(x = Absorbance, y = Phosphorus)) +
  geom_point(position = "jitter",size=0.05) + 
  geom_smooth(method='lm',se=FALSE) + 
  ggtitle('C:  Absorbance vs Phosphorus') +
  theme_cowplot()

```
###It is seen in the graph above that there is a linear trend between absorbance and phosphorus levels.
